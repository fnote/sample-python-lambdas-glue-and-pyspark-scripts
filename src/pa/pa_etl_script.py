...
def write_dataframe_to_s3(opco_id, file_name):
    print("starting uploading PA data of opco %s to s3 file %s" % (opco_id, file_name))
    key = output_file_path + file_name
...
def load_data(opco_id, df, cluster_id):
    ...
    conn = get_new_connection(connection_details["host"], connection_details["user"],
                              connection_details["decrypted"])
    ...
    load_qry = "LOAD DATA FROM S3 '" + s3_output_file_path + "' " \
...
def get_connection_details(cluster_id):
...
def get_new_connection(host, user, decrypted):
...
if __name__ == "__main__":
    args = getResolvedOptions(sys.argv, ['s3_input_bucket', 's3_input_file_key', 'etl_timestamp', 'etl_output_path_key',
                                         'INTERMEDIATE_S3_BUCKET', 'METADATA_LAMBDA', 'ENV',
    ...
    try:
        ...
        item_zone_prices_for_opco_in_cluster_1 = dict(
