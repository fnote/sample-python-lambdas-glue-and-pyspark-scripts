...
rm -rf .package PAAnalyzeWaitOrLoadClusterLambda.zip
mkdir .package

pip3 install --target .package -r requirements.txt

pushd .package > /dev/null
zip --recurse-paths ${SCRIPT_DIRECTORY}/PAAnalyzeWaitOrLoadClusterLambda.zip .
popd > /dev/null

zip --grow PAAnalyzeWaitOrLoadClusterLambda.zip index.py
