...
from move_s3_objects import copy_input_file, copy_objects_with_prefix, \
    delete_object, delete_directory
...
if __name__ == "__main__":
    ...
    decompressed_file_parsed_path = urlparse(
        decompressed_file_path, allow_fragments=False)
    ...
    copy_input_file(
        ...
        s3_input_file_key,
        archiving_s3_bucket,
        input_file_destination_key)

    # do this if gzip only
    if file_type == 'gz':
        copy_input_file(
            decompressed_file_bucket,
            decompressed_file_key,
            archiving_s3_bucket,
            decompressed_file_destination_key)
    ...
    copy_objects_with_prefix(
        intermediate_s3_bucket,
        partitioned_files_path,
        archiving_s3_bucket,
        opco_partitioned_path)
    ...
    delete_directory(
        source_bucket=intermediate_s3_bucket,
        prefix=partitioned_files_path)
    delete_directory(
        source_bucket=intermediate_s3_bucket,
        prefix=intermediate_directory_path)
