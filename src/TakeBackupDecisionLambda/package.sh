...
rm -rf .package TakeBackupDecisionLambda.zip
mkdir .package

pip3 install --target .package -r requirements.txt

pushd .package > /dev/null
zip --recurse-paths ${SCRIPT_DIRECTORY}/TakeBackupDecisionLambda.zip .
popd > /dev/null

zip --grow TakeBackupDecisionLambda.zip index.py
