...
rm -rf .package AnalyzeWaitOrLoadClusterLambda.zip
mkdir .package

pip3 install --target .package -r requirements.txt

pushd .package > /dev/null
zip --recurse-paths ${SCRIPT_DIRECTORY}/AnalyzeWaitOrLoadClusterLambda.zip .
popd > /dev/null

zip --grow AnalyzeWaitOrLoadClusterLambda.zip index.py
